...
describe("runPipeline", () => {
  function expectCollectionBeforeScore(): void {
    expect(mockRunCollection).toHaveBeenCalledTimes(1);
    expect(mockRunScore).toHaveBeenCalledTimes(1);
    expect(mockRunCollection.mock.invocationCallOrder[0]).toBeLessThan(mockRunScore.mock.invocationCallOrder[0]);
  }
