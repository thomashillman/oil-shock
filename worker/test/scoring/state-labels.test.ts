...
import type { Subscores } from "../../src/types";
...
describe("computeDislocationState", () => {
  it("returns aligned when score is low and physical pressure is low", () => {
    ...
    const result = computeDislocationState(0.2, subscores, freshFreshness, 0);
  ...
  it("returns mild_divergence when score is in mild range", () => {
    ...
    const result = computeDislocationState(0.45, subscores, freshFreshness, 0);
  ...
  it("returns persistent_divergence when score is high, duration > 3 days, all signals align", () => {
    ...
    const result = computeDislocationState(0.65, subscores, freshFreshness, durationSeconds);
  ...
  it("returns deep_divergence when score >= 0.75, all confirmations met, duration >= 5 days", () => {
    ...
    const result = computeDislocationState(0.75, subscores, freshFreshness, durationSeconds);
  ...
  it("downgrades to aligned when critical data is stale", () => {
    ...
    const result = computeDislocationState(0.75, subscores, staleFreshness, 86400);
  ...
  it("escalates to transmission_phase when transmission rises significantly", () => {
    ...
    const durationSeconds = 4 * 24 * 3600; // 4 days
    const result = computeDislocationState(0.7, subscores, freshFreshness, durationSeconds);
